...
function DropdownPanel({
  ...
  return (
    <div className="absolute top-[calc(100%+4px)] left-0 z-50 min-w-[176px] bg-[var(--wm-bg-02)] border border-[var(--wm-border-02)] rounded-[var(--radius-lg,8px)] py-1 shadow-xl">
      {items.map((item) => (
        <Link
          key={item.href}
          to={item.href}
  ...
  if (item.dropdown || item.panel) {
    return (
      <div ref={ref} className="relative">
        ...
        {/* Rich Earn panel */}
        {open && item.panel === 'earn' && (
          <EarnDropdown onClose={() => setOpen(false)} />
        )}

        {/* Rich Resources panel */}
        {open && item.panel === 'resources' && (
          <ResourcesDropdown onClose={() => setOpen(false)} />
        )}

        {/* Default simple panel */}
        {open && item.dropdown && (
          <DropdownPanel items={item.dropdown} onClose={() => setOpen(false)} />
        )}
      </div>
    )
  }
            ...
            {isConnected ? (
              ...
              <>
                ...
                <div ref={avatarMenuRef} className="relative shrink-0">
                  ...
                  {showUserMenu && (
                    <div className="absolute top-[calc(100%+8px)] right-0 z-50 animate-in fade-in zoom-in-95 duration-150 origin-top-right">
                      <UserMenuDropdown
                        address="GQ98...iA5Y"
